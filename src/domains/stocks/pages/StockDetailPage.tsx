...
import { Button, CapabilityUnavailable, ContractDriftState, EmptyState, ErrorState, LoadingState, PageHeader, StatusBadge, TradingDatePicker } from '@/shared/ui';
...
export function StockDetailPage() {
  ...
  return <div className="stack stock-detail-page">
    ...
    <div className="stock-context-toolbar"><TradingDatePicker label="as-of" value={selectedDate} max={dates.data?.marketWatermark ?? dates.data?.dates[0] ?? undefined} onChange={(value) => update('date', value)} aria-label="选择个股研究交易日" /><div className="stock-context-group"><span>复权</span><div className="segmented-control">{adjustments.map((item) => <button type="button" key={item.id} className={adjustment === item.id ? 'active' : ''} onClick={() => update('adjustment', item.id)}>{item.label}</button>)}</div></div><div className="stock-context-group"><span>窗口</span><div className="segmented-control">{ranges.map((item) => <button type="button" key={item.id} className={range === item.id ? 'active' : ''} onClick={() => update('range', item.id)}>{item.label}</button>)}</div></div><span className="stock-context-note">价格口径与窗口均由 URL 恢复；研报、调研只读取不晚于 as-of 的记录。</span></div>
