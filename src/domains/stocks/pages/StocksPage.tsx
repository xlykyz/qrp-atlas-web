...
import { Button, ErrorState, LoadingState, PageHeader, Panel, PanelBody, PanelHeader, StatusBadge } from '@/shared/ui';
...
export function StocksPage() {
  ...
  return <div className="stack stock-list-page">
    ...
    <Panel><PanelHeader title="股票筛选" meta="代码、名称、交易所与在市状态由后端过滤" /><PanelBody><div className="stock-filterbar">
      ...
      <label className="date-control"><CalendarDays size={14} /><span>研究日</span><select className="select" value={selectedDate} onChange={(event) => update('date', event.target.value)} aria-label="选择研究交易日">{(dates.data ?? [selectedDate]).map((date) => <option key={date} value={date}>{date}</option>)}</select></label>
    </div></PanelBody></Panel>
