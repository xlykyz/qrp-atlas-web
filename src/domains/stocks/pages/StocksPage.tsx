...
export function StocksPage() {
  ...
  const update = useCallback((key: 'date' | 'query' | 'exchange' | 'active' | 'page', value: string, replace = false) => setSearchParams((current) => {
  ...
  useEffect(() => {
    if (isComposingRef.current || inputQuery === keyword) return;
    const timer = setTimeout(() => {
      if (!isComposingRef.current) {
        update('query', inputQuery, true);
      }
    }, 250);
    return () => clearTimeout(timer);
  }, [inputQuery, keyword, update]);
  ...
  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setInputQuery(value);
    const nativeIsComposing = (event.nativeEvent as InputEvent)?.isComposing ?? false;
    if (!isComposingRef.current && !nativeIsComposing && !value) {
      update('query', '', true);
    }
  };
  ...
  return <div className="stack stock-list-page">
    ...
    <Panel><PanelHeader title="股票筛选" meta="代码、名称、交易所与在市状态由后端过滤" /><PanelBody><div className="stock-filterbar">
      <label className="market-search stock-search"><Search size={14} /><input value={inputQuery} onChange={handleInputChange} onCompositionStart={handleCompositionStart} onCompositionEnd={handleCompositionEnd} placeholder="输入代码或名称" aria-label="搜索股票" /></label>
